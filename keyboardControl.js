...
document.addEventListener('keydown', function(event) {
  switch (event.key) {
    case "w":
      CONTROLSTWO.tim.up = true;
      //console.log("tim move");
      break;
    case "s":
      CONTROLSTWO.tim.down = true;
      break;
    default:
      break;
    case "p":
...
document.addEventListener('keyup', function(event) {
  switch (event.key) {
    case "r":
      ...
      break;
    case "w":
      CONTROLSTWO.tim.up = false;
      break;
    case "s":
      CONTROLSTWO.tim.down = false;
      break;
    case "f":
     if (GAME.beanCountTwo >=5){
        CONTROLSTWO.tim.fart = true;
        for (var i =0;i < 40;i++)
        {
          Tim.bumParticles.push(new Particle(Tim.x + 10,Tim.y+20));
        }
        var num = Math.ceil(Math.random()*10);
        console.log(num);
        if (num == 1) fartSound1.play();
        ...
        GAME.beanCountTwo -= 5;
        setTimeout(function(){ CONTROLSTWO.tim.fart = false; }, 200);}
      break;
    case "k":
      Start();
      break;
